...
- 提供收集箱（inbox）队列与处理向导（Process Wizard），帮助用户把收集箱（inbox）笔记稳定转化为永久笔记（permanent），并在转化过程中建立至少 1 条关联链接。
...
5. **命令命名与固定顺序（中文 + 序号）**
   - 不在命令名称中包含插件名（Obsidian 命令面板会显示插件名）；核心工作流命令按 1–5 编号，以便在命令面板中形成固定顺序与肌肉记忆：
     - `1| 新建闪念笔记（fleeting）`
     - `2| 新建文献笔记（literature）`
     - `3| 打开收集箱（inbox）`
     - `4| 处理下一条 · 收集箱（inbox）`
     - `5| 处理当前笔记 · 升级为永久笔记（permanent）`
   - 非主流程的维护类命令使用 `工具 - ...`（例如 `工具 - 分配卡片 ID（当前笔记）`）。

6. **索引与性能**
   - 使用 `MetadataCache` 读取 frontmatter/tags/outlinks 信息，构建轻量索引：
     - `zk_id -> TFile`（用于唯一性校验）
     - 收集箱（inbox）列表（用于队列 UI）
...
10. **术语与文案一致化（中文 + token 常驻）**
   - 所有用户可见文案（命令、面板/Modal、设置、Notice、README）中，涉及 `zk_*` 概念时 MUST 使用“中文（token）”格式，避免只显示中文或只显示 token。
   - frontmatter 字段名在 UI 中以“中文（zk_key）”展示，以帮助用户建立概念联系：
     - 类型（`zk_type`）、状态（`zk_status`）、卡片 ID（`zk_id`）、来源（`zk_source`）
   - `zk_status` 的用户可见值：
     - 收集箱（`inbox`）、处理中（`processing`）、已完成（`done`）、已归档（`archived`）
   - `zk_type` 的用户可见值：
     - 闪念笔记（`fleeting`）、文献笔记（`literature`）、永久笔记（`permanent`）、索引笔记（`index`）、项目笔记（`project`）
