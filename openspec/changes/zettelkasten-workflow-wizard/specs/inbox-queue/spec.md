...
插件 MUST 提供命令打开 Inbox 队列界面，以便用户查看与选择待处理项。

#### Scenario: Open inbox UI
- **WHEN** 用户执行命令 `Zk: 3| 打开收集箱`
- **THEN** 插件 MUST 打开一个界面（例如 Modal）列出所有 inbox 项

### Requirement: Process next inbox item
插件 MUST 提供命令“一键处理下一条 inbox”，默认按最早创建优先（先进先出）选择待处理项。

#### Scenario: Process next item (FIFO)
- **WHEN** 用户执行命令 `Zk: 4| 处理下一条（收集箱）`
...
- **WHEN** 用户在 Inbox 队列中对某条项目执行“Archive”操作
- **THEN** 该文件的 `zk_status` MUST 被更新为 `archived`
- **THEN** 该文件 MUST 从 Inbox 队列中消失
